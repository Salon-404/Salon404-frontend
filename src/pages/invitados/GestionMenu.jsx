...
export const GestionMenu = () => {
  ...
  const handleGuardarPrecio = (dietTypeId) => {
    setPreciosLocales((prev) => ({
      ...prev,
      [dietTypeId]: parseFloat(nuevoPrecio) || 0,
    }));
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 xl:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {dietas.map((dieta) => (
              <div
                key={dieta.id}
                className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-[#B5853F] transition-colors"
              >
                ...
                {editingDietId === dieta.id ? (
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="w-24 border-2 border-[#B5853F] rounded p-1 text-sm font-bold focus:outline-none"
                      placeholder="Precio"
                    />
                    <button
                      onClick={() => handleGuardarPrecio(dieta.id)}
                      className="text-xs bg-[#B5853F] text-white font-bold px-3 py-1.5 rounded hover:bg-[#966D32] transition"
                    >
                      Ok
                    </button>
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    <span className="font-extrabold text-gray-900">
                      ${(preciosLocales[dieta.id] || 0).toLocaleString("es-AR")}
                    </span>
                    <button
                      onClick={() => {
                        setEditingDietId(dieta.id);
                        setNuevoPrecio(preciosLocales[dieta.id]);
                      }}
                      className="text-xs font-bold text-[#B5853F] hover:text-[#966D32] uppercase tracking-wider transition-colors"
                    >
                      Editar
                    </button>
                  </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 rounded-xl shadow-lg border-b-4 border-[#B5853F] flex flex-col justify-between">
          <div>
            <h2 className="text-sm font-bold text-[#B5853F] uppercase tracking-widest">
              Costo Total de Catering
            </h2>
            <p className="text-xs text-gray-400 mt-1 font-medium">
              (Calculado sobre invitados confirmados)
            </p>
          </div>
          <div className="mt-6">
            <span className="text-5xl font-black text-[#CBA05F] tracking-tight">
          ...
          <div className="text-xs font-semibold text-gray-300 mt-4 border-t border-gray-700 pt-3 flex justify-between">
            ...
            <span className="bg-gray-700 px-2 py-0.5 rounded text-[#CBA05F]">
              {resumen.reduce((acc, curr) => acc + curr.totalConfirmed, 0)}
            </span>
          </div>
        </div>
      </div>
