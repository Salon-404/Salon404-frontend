...
export const GestionMenu = () => {
  ...
  const handleGuardarPrecio = (dietTypeId) => {
    setPrecios((prev) =>
      prev.map((p) =>
        p.dietTypeId === dietTypeId
          ? { ...p, price: parseFloat(nuevoPrecio) || 0 }
          : p,
      ),
    );
  ...
  const costoCateringTotal = useMemo(() => {
    return invitados
      ...
      .reduce((total, inv) => {
        const precioDieta = precios.find(
          (p) => p.dietTypeId === inv.dietTypeId,
        );
  ...
  const resumenProveedor = useMemo(() => {
    return precios.map((p) => {
      const cantidadConfirmados = invitados.filter(
        (inv) => inv.guestStatusId === 2 && inv.dietTypeId === p.dietTypeId,
      ).length;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">
        Gestión del Menú y Catering
      </h1>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        {/* Panel de Precios */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 xl:col-span-2">
          <h2 className="text-lg font-extrabold text-gray-800 mb-4 uppercase tracking-wide">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {precios.map((p) => (
              <div
                key={p.dietTypeId}
                className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-amber-300 transition-colors"
              >
                ...
                {editingDietId === p.dietTypeId ? (
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="w-24 border-2 border-amber-500 rounded p-1 text-sm font-bold focus:outline-none"
                      placeholder="Precio"
                    />
                    <button
                      onClick={() => handleGuardarPrecio(p.dietTypeId)}
                      className="text-xs bg-amber-600 text-white font-bold px-3 py-1.5 rounded hover:bg-amber-700 transition"
                    >
                      Ok
                    </button>
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <button
                      onClick={() => {
                        setEditingDietId(p.dietTypeId);
                        setNuevoPrecio(p.price);
                      }}
                      className="text-xs font-bold text-amber-600 hover:text-amber-800 uppercase tracking-wider transition-colors"
                    >
                      Editar
                    </button>
                  </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 rounded-xl shadow-lg border-b-4 border-amber-500 flex flex-col justify-between">
          <div>
            <h2 className="text-sm font-bold text-amber-500 uppercase tracking-widest">
              Costo Total de Catering
            </h2>
            <p className="text-xs text-gray-400 mt-1 font-medium">
              (Calculado en base a invitados confirmados)
            </p>
          </div>
          <div className="mt-6">
            <span className="text-5xl font-black text-amber-400 tracking-tight">
              ${costoCateringTotal.toLocaleString("es-AR")}
            </span>
          </div>
          <div className="text-xs font-semibold text-gray-300 mt-4 border-t border-gray-700 pt-3 flex justify-between">
            ...
            <span className="bg-gray-700 px-2 py-0.5 rounded text-amber-400">
              {invitados.filter((i) => i.guestStatusId === 2).length}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border-t-4 border-gray-800 h-max">
          <div className="flex justify-between items-center mb-5 border-b pb-3">
            ...
            <button
              ...
              className="text-xs bg-gray-100 text-gray-700 font-bold px-3 py-1.5 rounded hover:bg-gray-200 transition"
            >
              Imprimir
            </button>
          </div>
          ...
          <div className="space-y-3">
            {resumenProveedor.map((res) => (
              ...
              >
                ...
                <span
                  className={`${res.cantidad > 0 ? "bg-amber-100 text-amber-800 border border-amber-200" : "bg-gray-200 text-gray-500"} px-3 py-1 rounded font-extrabold text-sm`}
                >
                  {res.cantidad} platos
                </span>
              </div>
          ...
          <div className="mt-6 bg-amber-50 border border-amber-200 rounded-lg p-4 text-xs text-amber-900 font-medium">
            <strong className="text-amber-700 uppercase tracking-wider block mb-1">
              Nota:
            </strong>
            Los platos a preparar reflejan únicamente a los invitados cuyo
