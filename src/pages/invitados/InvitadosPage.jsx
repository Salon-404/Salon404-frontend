...
export const InvitadosPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        toast.error(
          error.response.data.detail || "La mesa seleccionada está llena",
        );
      } else {
  ...
  const invitadosFiltrados = invitados.filter((inv) => {
    const cumpleStatus =
      filterStatus === "todos" || inv.guestStatusId === parseInt(filterStatus);
    const cumpleDiet =
      filterDiet === "todos" || inv.dietTypeId === parseInt(filterDiet);
    const cumpleMesa =
      filterMesa === "todos" ||
      (filterMesa === "sin_asignar"
        ? inv.tableId === null
        : inv.tableId === filterMesa);
    ...
    <div className="p-8 bg-gray-50 min-h-screen relative">
      ...
      <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-100 mb-6 flex flex-wrap gap-5 items-center">
        <div className="flex flex-col">
          ...
          <select
            ...
            className="border rounded-md p-2 text-sm bg-gray-50 focus:ring-[#B5853F] focus:border-[#B5853F]"
          >
      ...
      {isLoading ? (
        <div className="flex justify-center p-12">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#B5853F]"></div>
        </div>
      ) : (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-100">
              {invitadosFiltrados.map((invitado) => (
                <tr
                  key={invitado.id}
                  className="hover:bg-[#F8F1E7]/50 transition-colors"
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 inline-flex text-xs font-bold rounded-full ${invitado.guestStatusId === 2 ? "bg-green-100 text-green-800" : invitado.guestStatusId === 1 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"}`}
                    >
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    <span className="block font-medium">
                      {invitado.dietTypeName}
                    </span>
                    <span className="text-xs text-[#B5853F] font-semibold">
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <button
                      ...
                      className="text-[#B5853F] hover:text-[#966D32] font-bold mr-4"
                    >
                      Editar
                    </button>
                    <button
                      onClick={() => setInvitadoAEliminar(invitado.id)}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2.5 focus:ring-[#B5853F] focus:border-[#B5853F]"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="email"
                    {...register("email")}
                    className="mt-1 block w-full border rounded-md p-2.5 focus:ring-[#B5853F] focus:border-[#B5853F]"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700">
              ...
              <div>
                ...
                <select
                  {...register("tableId")}
                  className="mt-1 block w-full border border-[#B5853F] bg-[#F8F1E7] rounded-md p-2.5 focus:ring-[#B5853F] focus:border-[#B5853F] font-medium"
                >
                  <option value="">-- Sin asignar --</option>
                  {mesas.map((m) => (
                    <option key={m.id} value={m.id}>
                      {m.tableName} ({m.guests?.length || 0}/{m.capacity})
                    </option>
                  ))}
                </select>
              </div>
              <div className="flex justify-end space-x-3 mt-8 pt-4 border-t border-gray-100">
                <button
                  ...
                  onClick={cerrarModal}
                  disabled={isSaving}
                  className="px-5 py-2.5 bg-gray-100 text-gray-700 font-bold rounded-md hover:bg-gray-200"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isSaving}
                  className="px-5 py-2.5 bg-[#B5853F] text-white font-bold rounded-md hover:bg-[#966D32] shadow-md disabled:opacity-50"
                >
                  {isSaving
                    ? "Guardando..."
                    : editingInvitado
      ...
      {invitadoAEliminar && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm text-center border-t-4 border-red-500">
            <h3 className="text-xl font-extrabold text-gray-900 mb-2 mt-4">
              Eliminar Invitado
            </h3>
            ...
            <div className="flex justify-center space-x-3">
              <button
                onClick={() => setInvitadoAEliminar(null)}
                className="px-4 py-2 bg-gray-100 text-gray-800 rounded-md font-bold w-full"
              >
                Cancelar
              </button>
              <button
                onClick={confirmarEliminacion}
                className="px-4 py-2 bg-red-600 text-white rounded-md font-bold w-full shadow-md"
              >
                Sí, eliminar
              </button>
            </div>
